...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        UIEdgeInsets padding = UIEdgeInsetsMake(50, 10, 10, 10);
        [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {

            make.top.equalTo(self.textContainer).with.offset(padding.top); //with is an optional semantic filler
            make.left.equalTo(self.textContainer).with.offset(padding.left);
            make.bottom.equalTo(self.textContainer).with.offset(-padding.bottom);
            make.right.equalTo(self.textContainer).with.offset(-padding.right);
        }];
